...
describe('POS catering orders', function () {
    ...
    it('allows confirmed catering sale completion for catering-only companies', function (): void {
        [, $token, $companyId, $branchId] = financeActor();
        $accounts = posPostingAccounts($token, $companyId);
        $registerId = posCompletionRegister($token, $companyId, $branchId, $accounts['cash']);
        $variantId = posCompletionVariant($token, $companyId, 'POS-CAT-ONLY-1', 150000);
        posStockReceipt($token, $companyId, $branchId, $variantId, 2, 50000);
        $partner = Partner::create([
            'company_id' => $companyId,
            'type' => 'customer',
            'name' => 'SEKALORI Catering Customer',
            'code' => 'CAT-ONLY-001',
            'status' => 'active',
        ]);
